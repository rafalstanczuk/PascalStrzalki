...
begin
  ...
  SetColor(White);
  OutTextXY(100, 120, 'Click on this window and use:');
  OutTextXY(100, 135, 'a/z - Angle (+/- 10 degrees)');
  OutTextXY(100, 150, 's/d - Velocity (+/- 10 m/s)');
  OutTextXY(100, 165, 'ESC - Exit');

  writeln('Graphics ready. Click on the graphics window and use keyboard.');
end;
...
procedure DisplayParameters(inputStatus: string);
begin
  ...
  OutTextXY(50, 150, 'Status: ' + inputStatus);

  { Debug info }
  OutTextXY(50, 165, 'Driver: ' + GetDriverName);
  OutTextXY(50, 180, 'Mode: ' + IntToStr(GetGraphMode));
  OutTextXY(50, 195, 'MaxColor: ' + IntToStr(GetMaxColor));
end;
...
var ch: char;
    inputStatus: string;
begin
  { Try to read key with timeout - more reliable for graphics windows }
  inputStatus := 'Oczekiwanie na input...';

  if KeyPressed then
  begin
    ch := ReadKey;

    { Filter out null characters and control characters }
    if (Ord(ch) > 0) and (Ord(ch) < 255) then
    begin
      writeln('Valid key pressed: ', Ord(ch), ' (', ch, ')');  { Debug output }
      inputStatus := 'Klucz: ' + ch + ' (code: ' + IntToStr(Ord(ch)) + ')';

      case ch of
        'a', 'A': { Increase angle }
        begin
          writeln('Increasing angle from ', params.angle);
          inputStatus := 'Zwiększanie kąta z ' + IntToStr(params.angle);
          if params.angle < MAX_ANGLE then
          begin
            params.angle := params.angle + ANGLE_STEP;
            CalculateTrajectory;
            writeln('New angle: ', params.angle);
          end;
        end;

        'z', 'Z': { Decrease angle }
        begin
          writeln('Decreasing angle from ', params.angle);
          inputStatus := 'Zmniejszanie kąta z ' + IntToStr(params.angle);
        ...
        begin
          writeln('Decreasing velocity from ', params.velocity);
          inputStatus := 'Zmniejszanie prędkości z ' + IntToStr(params.velocity);
          if params.velocity > MIN_VELOCITY then
          begin
            params.velocity := params.velocity - VELOCITY_STEP;
            CalculateTrajectory;
            writeln('New velocity: ', params.velocity);
          end;
        end;

        'd', 'D': { Increase velocity }
        begin
          writeln('Increasing velocity from ', params.velocity);
          inputStatus := 'Zwiększanie prędkości z ' + IntToStr(params.velocity);
        ...
        begin
          writeln('Unhandled key: ', Ord(ch), ' (', ch, ')');
          inputStatus := 'Nieobsługiwany klawisz: ' + ch;
        end;
      end;
    end
    else
    begin
      writeln('Invalid key pressed: ', Ord(ch), ' (', ch, ')');  { Debug output }
      inputStatus := 'Nieprawidłowy klawisz (code: ' + IntToStr(Ord(ch)) + ')';
    end;
  end;

  { Return the status string for display }
  HandleInput := inputStatus;
end;
